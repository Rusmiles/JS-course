...
let x;
//Сложение +
(бинарное преобразует число к строке, есди есть строковое значение; унарное приводит строковое число к числу)
[+true == 1]
[+"" = 0]
...
//Вычитание -
(бинарный оператор приводит строковое значение к числу)
x = 9 - 5;
...
// (преобразуют операторы тольео в числа)
let resultOne = '25' - 5;
console.log(resultOne);
...
(Этот оператор используется крайне редко, иногда он используется 
для написания более короткого кода, поэтому мы говорим о нем, 
чтобы понимать, что при этом просиходит).
...
/* Чтобы избежать проблем при сравнении, нужно относиться
очень осторожно к любому сравнению с undefined / null,
...
разве что когда ты точно знаешь, что делаешь. 
...
3.	Если все операнды были истинными, возвращает значение
	последнего.
Короче, как оператор || только наоборот
*/
