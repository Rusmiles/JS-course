...
// Первый и последние элементы (firstChild / lastChild):
...
// Получаем коллекцию (псевдомассив) дочерних элементов childNodes:
...
// Получаем коллекцию дочерних узлов (childNodes):
const childNodes = bodyElement.childNodes;
...
узлами-элементами, которые представляют собой html-теги и
...
// Соседние (братские) и родительский элементы
...
/*
Самый универсальный метод поиска, он возвращает все элементы внутри elem, удовлетворящие данному CSS-селектору. Этот метод действительно мощный, потому что можно использовать любой CSS-селектор.
*/
/* 
// Поиск тега по селектору класса
...
const elemsSix =
	document.querySelectorAll('.lesson__list .lesson__item-list');
console.log(elemsSix);

// Поиск по ID (уникальный  атрибут)
const elemsSeven =
...
*/
// Т.о. методе querySelector в константу попадет не коллекция, а сразу один объект. Поэтому он работает быстрее, чем querySelectorAll
...
// Поиск можно вести не только в объекте document
/* 
...
// Живая коллекция отличается тем, что при изменении на лету дерева DOM она будет содержать в себе актуальную информацию, в то же время как querySelectorAll() будет содержать информацию на момент, когда мы вызвали, собственно, этот поиск
/* 
...
непосредственного родителя элемента, а метод closest ищет ближайшего родителя поднимаясь вверх по иерархии
*/
/* 
...
// Получение и изменение содержимого элемента с помощью свойства innerHTML
...
textElement.outerHTML = `<p>${textElementContent}</p> <p>Живи, а работай в
...
// Получение и изменение просто текста элемента методом textContent
...
// insertAdjacentHTML/Text/Element (insert - вставлять, вставить; adjacent - "примыкающий")
...
/*
"beforebegin" – вставить html непосредственно перед textElement (нашим объектом),
"afterbegin" – вставить html в начало textElement (внутрь объекта перед всем содержимым),
"beforeend" – вставить html в конец textElement (внутрь объекта после всего содержимого),
"afterend" – вставить html непосредственно после textElement (непосредственно после объекта).
*/
...
before/after/prepend/append – их быстрее написать,
и они могут вставлять как узлы, так и безопасный текст.
*/
//=========================================

// Перенос элемента (выполняется при помощи before / after / append / prepend)
...
// Получаем место назначения, куда нужно вставить клон
const lessonBlock = document.querySelector('.lesson');

// И определяем точное место вставки клона
...
// Добавить класс (.classList получает доступ к атрибуту class, а .add добавляет новый класс, не перезаписывая старый)
element.classList.add('active');
// Удалить класс (то же самое, только удаляет класс)
...
//Вычисленные стили. getComputedStyle(element, [pseudo]) ТОЛЬКО ДЛЯ ЧТЕНИЯ (вычислет / получает)
...
/* 
// Получаем элемент:
const otherElement = document.querySelector('.lesson__item-sub-list');

const elementBeforeOtherStyle = getComputedStyle(otherElement, "::before");
console.log(elementBeforeOtherStyle.backgroundColor);
 */
...
const marginLeft = element.style.marginLeft = "-20px";
...
// data-size-value (если имя состоит из 2 и < слов - то lowerCamelCase)
lessonText.dataset.sizeValue = '2323'
